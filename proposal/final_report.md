...
| 組員 | 負責內容 | 比例 |
| --- | --- | ---: |
| 曾怡瑄 | 遊戲畫面、素材整理、首頁與關卡 UI、報告與 Demo 輔助 | 50% |
| 陳嘉祥 | 三消邏輯、關卡資料、道具、特殊方塊、架構整理與除錯 | 50% |

## Demo 時可說明重點

如果老師詢問這個專案如何展現物件導向，可以回答：

> 我們把畫面物件、棋盤方塊、道具、關卡設定、遊戲流程拆成不同 class 或模組。例如 `Character` 是一般圖片物件，`GameCharacter` 繼承它並加入棋盤資料；`Item` 管理道具；`StageObject` 負責三消規則；`StageConfig` 統一管理關卡設定。這樣每個物件有自己的責任，後續新增關卡或修改規則比較不會互相影響。

如果老師詢問為什麼要拆 `.hpp` 和 `.cpp`，可以回答：

> `.hpp` 是宣告，讓其他檔案知道 class 有哪些函式可以用；`.cpp` 是實作，真正寫函式內容。這樣可以降低檔案耦合，也讓編譯和維護更清楚。

如果老師詢問消除邏輯，可以回答：

> 每個方塊都記錄自己的位置和六個鄰居。玩家滑動後，系統先交換兩個相鄰方塊，再檢查是否有三個以上相同顏色連線。如果有，就標記為消除、更新目標，再讓上方方塊掉落補位。掉落後會再檢查一次，形成連鎖消除。

如果老師詢問目前最有挑戰的地方，可以回答：

> 最難的是讓遊戲規則、動畫和輸入不要互相衝突。例如方塊正在掉落時不能讓玩家繼續交換，所以我們用階段狀態和動畫檢查來控制流程。
